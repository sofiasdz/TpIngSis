...
public class JSONFileWriter{
    ...
    public static void nodeListToJSON(List<ASTNode> tokenList, String testName) {
        Gson gson = new GsonBuilder().setPrettyPrinting().create();
        try {
            Writer writer = new FileWriter(testName+".txt");
            gson.toJson(tokenList,writer);
    ...
    public static List<ASTNode> fileJSONToNodeList(String testName){
        Gson gson = new Gson();
        try {
            Reader reader = new FileReader(testName+".txt");
            List<ASTNode> result = Arrays.asList(gson.fromJson(reader,ASTNode[].class));
            return result;
        } catch (FileNotFoundException e) {
