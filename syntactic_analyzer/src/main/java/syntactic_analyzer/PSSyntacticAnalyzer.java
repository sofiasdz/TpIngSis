...
public class PSSyntacticAnalyzer implements SyntacticAnalyzer {
    ...
    public ASTNode tokensToNode(List<Token> tokens) {
        ...
        for (int i = tokenList.size() - 1; i >= 0; i--) {
            if (tokenList.get(i).getType().equals(TokenType.LET) | tokenList.get(i).getType().equals(TokenType.SEMICOLON))
                tokenList.remove(i);
        }
    ...
    public ASTNode tokensToNodeRefactored(List<Token> tokens) {
        ...
        if (tokens.get(0).getType().equals(TokenType.LET)) {
            ...
            if (identifier.isEmpty() || type.isEmpty()) throw new RuntimeException("Invalid variable declaration");
            Optional<ASTNode> declaration = ASTNodeIdentifier(tokens.get(2), type.get(), identifier.get());
    ...
    ASTNode operationResolver(List<Token> tokens){
        TokenGroup literals = new TokenGroup(List.of(TokenType.STRING,TokenType.FLOATING_POINT,TokenType.INTEGER, TokenType.IDENTIFIER));
        if (tokens.size()==1 && literals.belongs(tokens.get(0))) return ASTNodeIdentifier(tokens.get(0)).get();

        //Shunting Yard
        Stack<Token> operatorStack = new Stack<>();
        Queue<Token> outputQueue = new ArrayDeque<>();
        TokenGroup operators = new TokenGroup(List.of(TokenType.SUBSTRACTION,TokenType.ADDITION,TokenType.DIVISION,TokenType.MULTIPLICATION));
        TokenGroup operands = new TokenGroup(List.of(TokenType.STRING,TokenType.INTEGER,TokenType.FLOATING_POINT,TokenType.IDENTIFIER));
        for (int i = 0; i < tokens.size(); i++) {
            if(operands.belongs(tokens.get(i))) {
                ...
                if(operatorStack.isEmpty()) {
                    ...
                    if(operatorPrecedence(operatorStack.peek())<operatorPrecedence(tokens.get(i))){
                        ...
                        while(!operatorStack.isEmpty() && operatorPrecedence(operatorStack.peek())>=operatorPrecedence(tokens.get(i))){
